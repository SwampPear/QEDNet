...
/-- declaring module via the environment’s module index (robust provenance) -/
...
def dumpAll : MetaM (Array Json) := do
  ...
  for (n, ci) in env.constants.map₁.toList do
    -- keep only Mathlib.* theorems/lemmas
...
def main : IO Unit := do
  ...
  -- pretty-print options (rc2 wants a full Core.Context with file fields)
